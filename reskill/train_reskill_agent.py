...
def train(agent, residual_agent, env, skill_vae, action_vae, skill_prior, logistic_C, logistic_k, 
    ...
    for epoch in tqdm(range(agent.epochs)):
        for t in range(local_steps_per_epoch):
            ...
            sampled_actions = action_vae.decode(torch.as_tensor(o, dtype=torch.float32), n)
            sample = AttrDict(noise=sampled_actions, state=o)
            # Warp noise vector to latent space skill
            z = skill_prior.inverse(sample).noise.detach()

            if proc_id() == 0:
                wandb.log({"n": n[0][0]}, env_step_cnt)
                wandb.log({"sample_action": sampled_actions[0][0]}, env_step_cnt)
...
def main():
    ...
    # Load Actions VAE module
    action_vae_path = "./results/saved_skill_models/" + args.dataset_name + "/action_vae.pth"
    action_vae = torch.load(action_vae_path, map_location=device)
