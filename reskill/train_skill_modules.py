...
from reskill.models.action_vae import ActionVAE
...
class ModelTrainer():
    def __init__(self, dataset_name, config_file):
        ...
        self.acvae_save_path = self.save_dir + "action_vae.pth"
        ...
        self.action_vae = ActionVAE(n_actions=conf.skill_vae.n_actions, n_obs=conf.skill_vae.n_obs, n_hidden=conf.skill_vae.n_hidden,
                                  seq_length=conf.skill_vae.subseq_len, n_z=conf.skill_vae.n_z, device=self.device).to(self.device)
        
        self.optimizer = optim.Adam(self.skill_vae.parameters(), lr=conf.skill_vae.lr)

        self.optimizer_ac = optim.Adam(self.action_vae.parameters(), lr=conf.action_vae.lr)

        self.sp_nvp = stacked_NVP(d=conf.skill_prior_nvp.d, k=conf.skill_prior_nvp.k, n_hidden=conf.skill_prior_nvp.n_hidden,
                                  state_size=conf.skill_vae.n_obs, action_size=conf.skill_vae.n_actions, n=conf.skill_prior_nvp.n_coupling_layers, device=self.device).to(self.device)
    ...
    def fit(self, epoch):
        ...
        for i, data in enumerate(self.train_loader):
            ...
            output = self.action_vae(data)
            losses_ac = self.action_vae.loss(data, output)
            loss = losses_ac.total_loss
            ...
            if i % 100:
                ...
                wandb.log({'AC_Loss_VAE':losses_ac.bc_loss.item()}, epoch)
                wandb.log({'ACKL_Loss_VAE':losses_ac.kld_loss.item()}, epoch)
                wandb.log({'NVP_Loss':sp_loss.item()}, epoch)
            
        train_loss = running_loss/len(self.train_loader.dataset)
        train_ac_loss = action_loss/len(self.train_loader.dataset)
        return train_loss, train_ac_loss
    ...
    def validate(self):
        ...
        with torch.no_grad():
            for i, data in enumerate(self.val_loader):
                ...
                loss = losses_ac.bc_loss.item()
                action_loss += loss

        val_loss = running_loss/len(self.val_loader.dataset)
        val_ac_loss = action_loss/len(self.val_loader.dataset)
        return val_loss, val_ac_loss


    def train(self):
        print("Training...") 
        for epoch in tqdm(range(self.n_epochs)):
            train_epoch_loss, train_ac_epoch_loss = self.fit(epoch)
            if epoch%5 == 0:
                val_epoch_loss, val_ac_epoch_loss = self.validate()

            wandb.log({'train_loss':train_epoch_loss}, epoch)
            wandb.log({'train_ac_loss':train_ac_epoch_loss}, epoch)
            wandb.log({'val_loss':val_epoch_loss}, epoch)
            wandb.log({'val_ac_loss':val_ac_epoch_loss}, epoch)

            if epoch % 50 == 0:
                torch.save(self.skill_vae, self.vae_save_path)
                torch.save(self.sp_nvp, self.sp_save_path)
                torch.save(self.action_vae, self.acvae_save_path)
