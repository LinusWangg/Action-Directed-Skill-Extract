...
class R_NVP(nn.Module):
    ...
    """
    def __init__(self, d, k, state_size, action_size, n_hidden, device):
        super().__init__()
        self.d, self.k = d, k
        self.device = device
        ...
        self.sig_net = nn.Sequential(
                    nn.Linear(k + state_size + action_size, n_hidden),
                    nn.LeakyReLU(),
    ...
    def forward(self, x, flip=False):
        ...
        # forward
        sig = self.sig_net(torch.cat((x1, x["state"], x["action"]), 1))
        z1, z2 = x1, x2 * torch.exp(sig) + self.mu_net(torch.cat((x1, x["state"], x["action"]), 1))
        ...
        return AttrDict(skill=z_hat, state=x["state"], action=x["action"]), log_pz, log_jacob
...
class stacked_NVP(nn.Module):
    def __init__(self, d, k, n_hidden, state_size, action_size, n, device):
        super().__init__()
        self.bijectors = nn.ModuleList([
            R_NVP(d, k, state_size, action_size, n_hidden=n_hidden, device=device).to(device) for _ in range(n)
        ])
